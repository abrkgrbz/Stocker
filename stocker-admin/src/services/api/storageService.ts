...
export const storageService = {
  ...
  async getAllBuckets(): Promise<BucketsResponse> {
    const response = await apiClient.get<BucketsResponse>('/admin/storage/buckets');
  ...
  async deleteBucket(bucketName: string): Promise<DeleteBucketResponse> {
    const response = await apiClient.delete<DeleteBucketResponse>(`/admin/storage/buckets/${encodeURIComponent(bucketName)}`);
    return response.data;
  },
  ...
  async deleteMultipleBuckets(bucketNames: string[]): Promise<DeleteMultipleBucketsResponse> {
    const response = await apiClient.post<DeleteMultipleBucketsResponse>('/admin/storage/buckets/delete-multiple', {
      bucketNames
    });
    return response.data;
  },
