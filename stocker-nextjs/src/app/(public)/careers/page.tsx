      ...
      <main>
        ...
        <section className="py-20 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto px-6">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-6">Kariyer Fırsatları</h1>
        ...
        <section className="py-16 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
              ...
              <p className="text-slate-500">Çalışanlarımıza sunduğumuz avantajlar</p>
            </motion.div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {benefits.map((benefit, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="p-6 bg-white rounded-2xl border border-slate-200">
        ...
        <section className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
              <h2 className="text-3xl font-bold text-slate-900 mb-4">Açık Pozisyonlar</h2>
              <p className="text-slate-500">{positions.length} açık pozisyon mevcut</p>
            </motion.div>
            <div className="space-y-4">
              {positions.map((position, index) => (
                <motion.div key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + index * 0.05 }} className="p-6 bg-white rounded-2xl border border-slate-200 hover:border-slate-300 transition-all">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-3 mb-2">
                        <h3 className="text-xl font-bold text-slate-900">{position.title}</h3>
                        <span className="px-3 py-1 bg-slate-100 text-slate-700 text-xs rounded-full">{position.department}</span>
                      </div>
                      <p className="text-slate-500 text-sm mb-3">{position.description}</p>
                      <div className="flex flex-wrap gap-2">
                        {position.requirements.map((req, i) => (
                          <span key={i} className="px-2 py-1 bg-slate-100 text-slate-700 text-xs rounded">{req}</span>
                        ))}
                      </div>
                    </div>
                    <div className="flex flex-col lg:items-end gap-2">
                      ...
                      <button className="px-6 py-2 bg-slate-900 hover:bg-slate-700 text-white text-sm font-medium rounded-xl transition-colors">
                        Başvur
                      </button>
                    </div>
                  </div>
        ...
        <section className="py-20">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8 bg-slate-50 rounded-2xl border border-slate-200">
              <h2 className="text-2xl font-bold text-slate-900 mb-4">Aradığınız pozisyonu bulamadınız mı?</h2>
              <p className="text-slate-500 mb-6">Genel başvurunuzu gönderin, size uygun bir pozisyon açıldığında iletişime geçelim.</p>
              <Link href="/contact" className="inline-block px-8 py-3 bg-slate-900 text-white font-medium rounded-xl hover:bg-slate-700 transition-colors">
                Genel Başvuru Yap
              </Link>
            </motion.div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between text-sm text-slate-500">
            ...
            <div className="flex items-center space-x-6 mt-4 md:mt-0">
              ...
              <Link href="/careers" className="text-slate-900">Kariyer</Link>
            </div>
          </div>
        </div>
      </footer>
