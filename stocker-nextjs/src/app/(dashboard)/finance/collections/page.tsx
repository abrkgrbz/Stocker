...
/**
 * Collections (Tahsilatlar) List Page
...
export default function CollectionsPage() {
  ...
  const handleDelete = async (paymentId: number) => {
    try {
      ...
      showSuccess('Tahsilat başarıyla silindi!');
    } catch (err) {
      showApiError(err, 'Tahsilat silinirken bir hata oluştu');
      throw err;
    }
  };

  const handleDeleteClick = (collection: PaymentSummaryDto) => {
    Modal.confirm({
      title: 'Tahsilatı Sil',
      content: `${collection.paymentNumber} numaralı tahsilat silinecek. Bu işlem geri alınamaz.`,
      okText: 'Sil',
      cancelText: 'İptal',
  ...
  const handleCreate = () => {
    router.push('/finance/collections/new');
  };

  const handleView = (collectionId: number) => {
    router.push(`/finance/payments/${collectionId}`);
  };
    ...
    <div className="min-h-screen bg-slate-50 p-8">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">Müşterilerden yapılan tahsilatları yönetin</p>
            </div>
            <div className="flex items-center gap-2">
              <Button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mt-1">Toplam Tahsilat</div>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-white border border-slate-300 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-medium text-slate-900">Tahsilatlar yüklenemedi</h3>
              <p className="text-xs text-slate-500">
                {error instanceof Error ? error.message : 'Tahsilatlar getirilirken bir hata oluştu.'}
              </p>
            </div>
            <Button
              size="small"
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="md:col-span-2">
            <Input
              placeholder="Tahsilat ara... (tahsilat no, müşteri)"
