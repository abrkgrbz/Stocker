...
import { ArrowLeftOutlined, SaveOutlined, DollarOutlined, UserOutlined, MinusCircleOutlined, PlusCircleOutlined } from '@ant-design/icons';
      ...
      <div className="sticky top-0 z-50 px-8 py-4" style={{ background: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(0, 0, 0, 0.06)' }}>
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900 m-0">Yeni Bordro</h1>
              <p className="text-sm text-gray-400 m-0">Calisan bordrosu olusturun</p>
            </div>
          </div>
          <Space>
            <Button onClick={() => router.push('/hr/payslips')}>Vazgec</Button>
            <Button type="primary" icon={<SaveOutlined />} loading={createPayslip.isPending} onClick={() => form.submit()} style={{ background: '#1a1a1a', borderColor: '#1a1a1a' }}>Kaydet</Button>
          </Space>
        </div>
      </div>
      ...
      <div className="px-8 py-8 max-w-7xl mx-auto">
        <Form form={form} layout="vertical" onFinish={handleSubmit} initialValues={{ status: 'Draft' }}>
          <Row gutter={48}>
            ...
            <Col xs={24} lg={14}>
              <div className="mb-8">
                <Text className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-3 block"><UserOutlined className="mr-1" /> Calisan & Donem Bilgileri</Text>
              ...
              <div className="mb-8">
                <Text className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-3 block"><PlusCircleOutlined className="mr-1" /> Kazanclar</Text>
                <Row gutter={16}>
