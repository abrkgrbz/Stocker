...
import { useEmployeeSkill, useUpdateEmployeeSkill, useEmployees } from '@/lib/api/hooks/useHR';
...
export default function EditEmployeeSkillPage() {
      ...
      <div className="sticky top-0 z-50 px-8 py-4" style={{ background: 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(0, 0, 0, 0.06)' }}>
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900 m-0">Yetkinlik Duzenle</h1>
              <p className="text-sm text-gray-400 m-0">{skill?.skillName}</p>
            </div>
          </div>
          <Space>
            <Button onClick={() => router.push(`/hr/employee-skills/${id}`)}>Vazgec</Button>
            <Button type="primary" icon={<CheckIcon className="w-4 h-4" />} loading={updateSkill.isPending} onClick={() => form.submit()} style={{ background: '#1a1a1a', borderColor: '#1a1a1a' }}>Kaydet</Button>
          </Space>
        </div>
      </div>
            ...
            <Col xs={24} lg={14}>
              <div className="mb-8">
                ...
                <Row gutter={16}>
                  ...
                  <Col span={12}><Form.Item name="lastUsedDate" className="mb-3"><DatePicker style={{ width: '100%' }} format="DD.MM.YYYY" /></Form.Item></Col>
                </Row>
              </div>
              ...
              <div className="mb-8">
                ...
                <Row gutter={16}>
                  <Col span={12}><Form.Item name="certificationDate" className="mb-3"><DatePicker style={{ width: '100%' }} format="DD.MM.YYYY" /></Form.Item></Col>
                  <Col span={12}><Form.Item name="certificationExpiry" className="mb-3"><DatePicker style={{ width: '100%' }} format="DD.MM.YYYY" /></Form.Item></Col>
                </Row>
