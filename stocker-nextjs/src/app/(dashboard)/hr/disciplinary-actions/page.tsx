...
export default function DisciplinaryActionsPage() {
  ...
  const handleDelete = async (action: DisciplinaryActionDto) => {
    try {
      ...
      showSuccess('Disiplin islemi basariyla silindi');
      refetch();
    } catch (err) {
      showApiError(err, 'Disiplin islemi silinemedi');
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-semibold text-slate-900">Disiplin Islemleri</h1>
            <p className="text-sm text-slate-500">Calisan disiplin sureclerini takip edin ve yonetin</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-sm text-slate-500">{totalCount} islem</span>
          <Button
      ...
      {error && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-slate-900">Disiplin islemleri yuklenemedi</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-slate-100">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Disiplin islemi ara... (calisan, islem turu)"
              value={searchText}
