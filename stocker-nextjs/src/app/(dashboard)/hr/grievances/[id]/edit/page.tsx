...
export default function EditGrievancePage() {
  ...
  useEffect(() => {
    if (grievance) {
      form.setFieldsValue({ ...grievance, filedDate: grievance.filedDate ? dayjs(grievance.filedDate) : null, resolutionDate: grievance.resolutionDate ? dayjs(grievance.resolutionDate) : null });
    }
  }, [grievance, form]);

  const handleSubmit = async (values: any) => {
    try {
      const data = { ...values, filedDate: values.filedDate?.toISOString(), resolutionDate: values.resolutionDate?.toISOString() };
