...
import { Card, Descriptions, Tag, Row, Col, Progress, Avatar } from 'antd';
import {
  PencilIcon,
  StarIcon,
  UserGroupIcon,
...
export default function SuccessionPlanDetailPage() {
  ...
  return (
    <DetailPageLayout
      title="Yedekleme Plani Detayi"
      subtitle={plan?.positionTitle}
      backPath="/hr/succession-plans"
      icon={<StarIcon className="w-5 h-5 text-white" />}
      iconBgColor="bg-orange-600"
      isLoading={isLoading}
      isError={isError || !plan}
      errorMessage="Plan Bulunamadi"
      errorDescription="Istenen yedekleme plani bulunamadi."
      statusBadge={
        plan && (
      ...
      actions={
        <Button
          ...
          onClick={() => router.push(`/hr/succession-plans/${id}/edit`)}
        >
          Duzenle
        </Button>
      }
    >
      <Row gutter={[24, 24]}>
        <Col xs={24} lg={8}>
          ...
          <Card className="mt-4" title="Mevcut Pozisyon Sahibi">
            <div className="flex items-center gap-3">
              <Avatar size={48} icon={<UserIcon className="w-4 h-4" />} />
              <div>
        ...
        <Col xs={24} lg={16}>
          ...
          <Card title={<><UserGroupIcon className="w-4 h-4 mr-2 inline-block" />Plan Bilgileri</>} className="mb-4">
            <Descriptions column={2} bordered size="small">
