...
export async function middleware(request: NextRequest) {
  ...
  // ==============================================
  // CMS PREVIEW MODE HANDLING
  // ==============================================

  // Handle /preview route - Enable draft mode and redirect
  if (pathname === '/preview') {
    ...
    // Create response with draft mode cookie
    const response = NextResponse.redirect(new URL(redirectUrl, request.url))

    // Set the __prerender_bypass cookie to enable draft mode
    // This is what draftMode().enable() does internally
    response.cookies.set('__prerender_bypass', process.env.__NEXT_PREVIEW_MODE_ID || 'preview-mode', {
      httpOnly: true,
      sameSite: 'none',
      secure: true,
      path: '/',
    ...
    // Also set the Next.js draft mode encryption key cookie
    response.cookies.set('__next_preview_data', process.env.__NEXT_PREVIEW_MODE_ENCRYPTION_KEY || '', {
      httpOnly: true,
      sameSite: 'none',
      secure: true,
      path: '/',
    ...
    console.log('[CMS Preview] Enabling draft mode, redirecting to:', redirectUrl)
    return response
  }

  // Handle /exit-preview route - Disable draft mode
  if (pathname === '/exit-preview') {
    const redirectUrl = request.nextUrl.searchParams.get('redirect') || '/'

    const response = NextResponse.redirect(new URL(redirectUrl, request.url))

    // Clear draft mode cookies
    response.cookies.delete('__prerender_bypass')
    response.cookies.delete('__next_preview_data')

    console.log('[CMS Preview] Disabling draft mode')
    return response
  }
